...
const SIMILAR_PRODUCTS = document.querySelector("#similarProduct");
      ...
      <a href="./detail.html?id=${item?.id}">
        ...
        ${
          item?.sale
            ? `
        <div
          class="absolute top-[11px] left-[11px] bg-[#f14f44] h-[28px] text-[14px] leading-[28px] font-medium uppercase px-[14px] rounded-[25px] text-white"
        >
          ${item.sale}
        </div>
        `
...
<div
class="price mt-[9px] text-[14px] leading-[30px] font-manrope flex items-center gap-2"
>/
${
  item.discount
    ? `<span class="text-[#f14f44] font-bold">$${item.discount}</span>
       ${
         item.price
           ? `<span class="text-[#9b9b9b] line-through">$${item.price}</span>`
           : ""
       }`
    : item.price
    ? `<span class="text-[#111111] font-bold">$${item.price}</span>`
    : ""
}
/
</div>
